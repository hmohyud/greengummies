...
/* hero */
.hero { padding: 72px 0 76px; overflow-x: clip; } /* sparks past the rim must not widen the page */
...
(function () {
  ...
  if (ring && grip) {
    ...
    var heatShown = 0, heatLevel = 0, whiteLevel = 0, whiteShown = 0, warmup = 0;
    (function spinLoop() {
      var now = performance.now();
      ...
      // friction heat with thermal inertia: hitting speed does not glow
      // instantly — friction needs time, so heating authority ramps in over
      // ~0.8s of sustained speed (warmup). Once warm the rim blooms fast,
      // but COOLS slowly, keeping an ember for a while after the spin dies.
      ...
      if (heatEl && !reduced) {
        ...
        warmup = target > 0.05 ? Math.min(1, warmup + 0.02 * f) : Math.max(0, warmup - 0.01 * f);
        target *= warmup;
