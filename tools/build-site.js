...
.plate .card { position: absolute; top: 50%; left: 50%; transform: translate(-50%,-50%); width: 57%; aspect-ratio: 1; filter: drop-shadow(0 16px 30px rgba(0,0,0,0.5)); }
.plate .card svg, .plate .card img { width: 100%; height: 100%; object-fit: contain; display: block; }
.plate-ring { position: absolute; inset: 0; width: 100%; height: 100%; cursor: grab; touch-action: none; will-change: transform; }
.plate-ring.dragging { cursor: grabbing; }
...
.rig-btn { display: block; width: 100%; background: none; border: 0; cursor: pointer; padding: 0; -webkit-tap-highlight-color: transparent; }
...
(function () {
  ...
  if (ring) {
    ...
    ring.addEventListener('pointermove', function (e) {
      ...
      angle += d;
      var now = performance.now();
      vel = Math.max(-14, Math.min(14, d / Math.max(1, now - lastT) * 16.7));
      lastA = a; lastT = now;
